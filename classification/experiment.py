...
for val in vals:
    ...
    for i in range(150):
        ...
        exp = explainer.explain_instance(
            x_test.values[i], wrapper.scale_predict, num_features=val)
        print(y_test.values[i])
        print(exp.as_list())
        l = x_test.values[i].astype(str).tolist()
        es = ','.join([a for a, b in exp.as_list()])
        row = str(i+1)+',[' + ' '.join(l) + '],,' + es + ',,' + str(np.argmax(wrapper.scale_predict(
            x_test.values[i].reshape(1, -1)))) + ',,' + str(np.argmax(y_test.values[i]))
        rows.append(row + "\n")
